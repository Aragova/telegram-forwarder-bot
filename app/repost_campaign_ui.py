...
def _format_invite_links_vip_status(settings: dict | None) -> str:
    if settings is None:
        return "⚠️ статус недоступен"
    return "🟢 включено" if bool(settings.get("enabled")) else "🔴 выключено"


def _format_invite_links_status(settings: dict | None) -> str:
    if settings is None:
        return "⚠️ статус недоступен"
    return "🟢 включено" if bool(settings.get("enabled")) else "🔴 выключено"


def _format_invite_links_destination(settings: dict | None) -> str:
    settings = settings or {}
    title = str(settings.get("destination_chat_title") or "").strip()
    if title:
        return f"👉 {title}"
...
def _format_invite_links_link_mode(settings: dict | None) -> str:
    value = str((settings or {}).get("link_mode") or "").strip()
...
def _format_invite_links_injection_mode(settings: dict | None) -> str:
    value = str((settings or {}).get("injection_mode") or "").strip()
    if value == "placeholder":
        return "{invite_link}"
    if value == "append_footer":
...
def _format_invite_links_per_target(settings: dict | None) -> str:
    return "🟢 отдельная ссылка для каждого рекламного канала" if bool((settings or {}).get("per_target_links_enabled")) else "⚪ одна ссылка на весь рекламный запуск"


def _format_invite_links_preview(settings: dict | None) -> str:
    settings = settings or {}
    if settings.get("preview_checked_at"):
        return "✅ проверен"
    return "обязателен" if bool(settings.get("preview_required")) else "не обязателен"


def build_repost_campaign_vip_features_view(*, rule_id: int, invite_links_settings: dict | None = None) -> tuple[str, InlineKeyboardMarkup]:
    invite_links_status = _format_invite_links_vip_status(invite_links_settings)
    text = (
        "💎 VIP-функции\n\n"
        "Дополнительные возможности для аккуратного запуска рекламных постов.\n\n"
        "🕒 Запланированные посты\n"
        "Планируйте отдельные рекламные посты заранее. "
        "Пользователь сам выбирает материал, время запуска и срок показа.\n\n"
        "🧹 Чистый канал\n"
        "Помогает не запускать новую базовую рекламу поверх активной рекламы. "
        "Если включён, запланированный запуск может подождать освобождения канала.\n\n"
        "📌 Время в топе\n"
        "Будет удерживать обычный автопостинг ViMi после публикации рекламного поста, "
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🕒 Запланированные посты", callback_data=f"rule_repost_campaign_scheduled_posts:{rule_id}")],
...
def build_repost_campaign_invite_links_view(*, rule_id: int, settings: dict | None, stats: dict | None = None) -> tuple[str, InlineKeyboardMarkup]:
    ...
    text = (
        "📥 Заявки и вступления\n\n"
        "ViMi будет создавать Telegram-ссылки для рекламных постов и считать результат рекламы:\n\n"
        "📥 заявки\n"
        "✅ вступления\n"
        "📊 конверсию\n\n"
        "ViMi не принимает и не отклоняет заявки.\n"
        "Если у вас уже есть бот автоприёма, он продолжит работать как раньше.\n\n"
        f"Статус: {_format_invite_links_status(settings)}\n\n"
...
def build_repost_campaign_active_placements_view(
    *,
    ...
    for placement in visible_placements:
        ...
        resolve_callback = (placement or {}).get("delete_failures_resolve_callback_data") or f"rule_repost_campaign_run_delete_failures_resolve_confirm:{rule_id}:{run_id}"
        report_callback = (placement or {}).get("report_callback_data") or f"rule_repost_campaign_views_report:{rule_id}:{run_id}"
        delete_failed = _safe_non_negative_int((placement or {}).get("delete_failed"))
        delete_pending = _safe_non_negative_int((placement or {}).get("delete_pending"))
        delete_processing = _safe_non_negative_int((placement or {}).get("delete_processing"))
        has_delete_counts = any(key in (placement or {}) for key in ("delete_pending", "delete_processing", "delete_failed"))
        action_row = [InlineKeyboardButton(text=f"📄 #{run_id}", callback_data=details_callback)]
        if not has_delete_counts or delete_pending + delete_processing > 0:
            action_row.append(InlineKeyboardButton(text=f"🧹 Удалить #{run_id}", callback_data=delete_callback))
        rows.append(action_row)
        if delete_failed > 0:
            rows.append([InlineKeyboardButton(text=f"🧯 Снять проблему #{run_id}", callback_data=resolve_callback)])
...
def build_repost_campaign_top_time_intro_view(*, rule_id: int) -> tuple[str, InlineKeyboardMarkup]:
    text = (
        "📌 Время в топе\n\n"
        "Будущая VIP-функция для рекламной кампании.\n\n"
        "После публикации рекламного поста ViMi сможет временно удерживать обычный автопостинг "
        "в выбранных каналах и группах.\n\n"
        "Что будет удерживаться:\n"
        "✅ автоматические репосты\n"
        "✅ автоматические видеопосты\n"
        "✅ публикации обычных правил по расписанию\n\n"
        "Что не будет блокироваться:\n"
        "ℹ️ “Отправить сейчас”\n"
        ...
        "1. Рекламный пост публикуется в выбранные каналы.\n"
        "2. Если “Время в топе” включено, ViMi создаёт паузу обычного автопостинга.\n"
        "3. После окончания паузы обычный постинг продолжится сам.\n\n"
        "Важно:\n"
...
def build_repost_campaign_vip_coming_soon_view(*, rule_id: int, feature: str | None = None) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if feature == "ab_test":
        text = (
            ...
            "Она поможет сравнивать два варианта рекламного поста и смотреть, "
            "какой вариант набирает больше просмотров."
        )
    else:
        text = "💎 Скоро в VIP-функциях\n\nЭта функция появится в следующих обновлениях ViMi."
...
def build_repost_campaign_run_delete_failures_resolve_confirm_view(*, rule_id: int, run_id: int, details: dict) -> tuple[str, InlineKeyboardMarkup]:
    summary = (details or {}).get("summary") or {}
    pending_processing = int(summary.get("delete_pending") or 0) + int(summary.get("delete_processing") or 0)
    lines = [
        "🧯 Снять проблему удаления",
        "",
        f"Запуск #{run_id}",
        "",
        "У этого запуска есть ошибки удаления.",
        "",
        "Используйте это действие только если:",
        "— канал/группа уже удалены;",
        "— пост уже исчез;",
        "— бот больше не имеет доступа к каналу;",
        "— удалить пост через Telegram технически невозможно.",
        "",
        "ViMi пометит проблемные публикации как технически завершённые.",
        "Это снимет ошибку удаления.",
        "",
        "Публикации в Telegram удаляться не будут.",
    ]
    if pending_processing > 0:
        lines.extend([
            "",
            "⚠️ В этом запуске ещё есть публикации, которые ожидают удаления.",
            "",
            "“Снять проблему” исправит только ошибки удаления.",
            "Ожидающие публикации останутся активными и всё ещё могут блокировать “Чистый канал”.",
            "",
            "Чтобы полностью убрать запуск, сначала попробуйте:",
            "🧹 Удалить запуск",
        ])
    rows = [
        [InlineKeyboardButton(text="✅ Снять проблему", callback_data=f"rule_repost_campaign_run_delete_failures_resolve_apply:{rule_id}:{run_id}")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data=f"rule_repost_campaign_active_placements:{rule_id}:0")],
    ]
    return "\n".join(lines), InlineKeyboardMarkup(inline_keyboard=rows)


def build_repost_campaign_run_delete_failures_resolve_result_view(*, rule_id: int, run_id: int, result) -> tuple[str, InlineKeyboardMarkup]:
    payload = result.to_dict() if hasattr(result, "to_dict") else dict(result or {})
    extra = payload.get("extra") or {}
    if payload.get("ok") and extra.get("noop"):
        text = "✅ Проблем удаления не найдено\n\nЭтот запуск уже не блокирует “Чистый канал” ошибками удаления."
    elif payload.get("ok"):
        text = (
            "✅ Проблема удаления снята\n\n"
            f"Запуск #{run_id}\n\n"
            f"Исправлено ошибок удаления: {int(extra.get('resolved') or 0)}\n"
            f"Осталось ожидающих удаления: {int(extra.get('remaining_pending') or 0)}\n"
            f"Осталось в процессе удаления: {int(extra.get('remaining_processing') or 0)}\n"
            f"Осталось ошибок удаления: {int(extra.get('remaining_failed') or 0)}"
        )
        if extra.get("still_active"):
            text += "\n\nЗапуск всё ещё активен, потому что часть публикаций ожидает удаления."
        else:
            text += "\n\nТеперь этот запуск больше не блокирует “Чистый канал”."
    else:
        error_text = payload.get("error_text") or "Попробуйте позже."
        text = f"⚠️ Не удалось снять проблему удаления\n\n{error_text}"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🧹 Активные размещения", callback_data=f"rule_repost_campaign_active_placements:{rule_id}:0")],
        [InlineKeyboardButton(text="💰 К кампании", callback_data=f"rule_repost_campaign_menu:{rule_id}")],
    ])
