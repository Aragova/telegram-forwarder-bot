...
def register_admin_reaction_handlers(dp: Dispatcher, ctx: AdminHandlersContext) -> None:
    ...
    async def handle_rule_reactions(callback: CallbackQuery):
        ...
        await ctx.edit_message_text_safe(message=callback.message, text=build_rule_reactions_text(rule_id, settings_row, accounts), reply_markup=build_rule_reactions_keyboard(rule_id, bool(settings_row and settings_row.get("enabled"))))
    ...
    async def handle_rule_reactions_add_account(callback: CallbackQuery):
        ...
        if not rule:
            ...
            return
        tenant_id = await ctx.run_db(_resolve_reaction_tenant_id, ctx, user_id, rule)
        ctx.user_states[user_id] = {"state": "reaction_auth_wait_phone", "rule_id": rule_id, "tenant_id": tenant_id}
        ctx.logger.info("REACTION_AUTH_STARTED | tenant_id=%s | rule_id=%s | user_id=%s", tenant_id, rule_id, user_id)
        await ctx.answer_callback_safe_once(callback)
    ...
    @dp.callback_query(lambda c: c.data.startswith("rule_reactions_auth_cancel:"))
    async def handle_rule_reactions_auth_cancel(callback: CallbackQuery):
        rule_id = int(callback.data.split(":", 1)[1])
        user_id = callback.from_user.id if callback.from_user else 0
        ...
        if tenant_id:
            ...
            ctx.logger.info("REACTION_AUTH_CANCELLED | tenant_id=%s | rule_id=%s | user_id=%s", tenant_id, rule_id, user_id)
        ctx.user_states.pop(user_id, None)
        callback.data = f"rule_reactions:{rule_id}"
        await handle_rule_reactions(callback)

    @dp.message(lambda m: m.from_user is not None and (ctx.user_states.get(m.from_user.id) or {}).get("state") in {"reaction_auth_wait_phone", "reaction_auth_wait_code", "reaction_auth_wait_password"})
    async def handle_reaction_auth_messages(message: Message):
    ...
    async def handle_rule_reactions_toggle(callback: CallbackQuery):
        ...
        await ctx.run_db(ctx.db.upsert_rule_reaction_settings, tenant_id=tenant_id, rule_id=rule_id, enabled=next_enabled, mode=str((settings_row or {}).get("mode") or "premium_then_normal"), preset=(settings_row or {}).get("preset_json"), max_accounts_per_post=int((settings_row or {}).get("max_accounts_per_post") or 3), delay_min_sec=int((settings_row or {}).get("delay_min_sec") or 3), delay_max_sec=int((settings_row or {}).get("delay_max_sec") or 30), premium_first=bool((settings_row or {}).get("premium_first", True)), stop_after_premium_success=bool((settings_row or {}).get("stop_after_premium_success", False)))
        callback.data = f"rule_reactions:{rule_id}"
        await handle_rule_reactions(callback)
