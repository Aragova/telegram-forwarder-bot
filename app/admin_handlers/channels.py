...
def register_admin_channel_handlers(dp: Dispatcher, ctx: AdminHandlersContext) -> None:
    ...
    async def handle_channels_menu(message: Message):
        ...
        if not await ctx.is_admin(message):
            return
        await ctx.send_message_safe(chat_id=message.chat.id, text="📡 Раздел: Каналы", reply_markup=ctx.get_channels_menu())
    ...
    async def handle_add_channel(message: Message):
        if message.text == "➕ Добавить источник":
            ...
            await ctx.send_message_safe(chat_id=message.chat.id, text="Выберите: канал или группа с темой", reply_markup=ctx.get_entity_kind_keyboard())
            return
        ...
        if not await ctx.is_admin(message):
            return
        await ctx.send_message_safe(chat_id=message.chat.id, text="Выберите тип записи", reply_markup=ctx.get_channel_type_keyboard())
    ...
    async def handle_entity_kind(message: Message):
        ...
        if state["action"] == "choose_source_kind":
            ...
            await ctx.send_message_safe(chat_id=message.chat.id, text="Отправьте ID канала" if message.text == "📺 Канал" else "Отправьте ID группы", reply_markup=ctx.get_cancel_keyboard())
    ...
    async def handle_chat_id_inputs(message: Message):
        ...
        if action in {"add_source_channel", "add_target_channel"}:
            ...
            try:
                ...
                if exists:
                    await ctx.send_message_safe(chat_id=message.chat.id, text="Такая запись уже есть", reply_markup=ctx.get_main_menu())
                else:
                    ...
                    if not created:
                        await ctx.send_message_safe(chat_id=message.chat.id, text="⚠️ Не удалось добавить запись в базу", reply_markup=ctx.get_main_menu())
                    else:
                        await ctx.send_message_safe(chat_id=message.chat.id, text=f"✅ Добавлен {'источник' if channel_type == 'source' else 'получатель'}: {title}", reply_markup=ctx.get_main_menu())
                ...
                await ctx.send_message_safe(chat_id=message.chat.id, text=f"❌ Ошибка доступа к каналу/чату: {exc}", reply_markup=ctx.get_main_menu())
            finally:
        ...
        if action in {"add_source_group", "add_target_group"}:
            try:
                ...
                await ctx.send_message_safe(chat_id=message.chat.id, text="Теперь отправьте ID темы", reply_markup=ctx.get_cancel_keyboard())
            except Exception as exc:
                ctx.logger.exception("Ошибка доступа к группе перед вводом thread_id | action=%s | chat_id=%s | error=%s", action, chat_id, exc)
                await ctx.send_message_safe(chat_id=message.chat.id, text=f"❌ Не удалось получить доступ к группе: {exc}", reply_markup=ctx.get_main_menu())
                ctx.reset_user_state(message.from_user.id)
            return
    ...
    async def handle_list_channels(message: Message):
        ...
        if not rows:
            await ctx.send_message_safe(chat_id=message.chat.id, text="Нет каналов", reply_markup=ctx.get_main_menu())
            return
        text = "📜 **СПИСОК КАНАЛОВ**\n\n"
        ...
        await ctx.send_message_safe(chat_id=message.chat.id, text=text[:4000], parse_mode="Markdown", reply_markup=ctx.get_main_menu())
    ...
    async def handle_remove_channel(message: Message):
        ...
        await ctx.send_message_safe(chat_id=message.chat.id, text=text[:4000], reply_markup=ctx.reply_keyboard_markup_cls(keyboard=keyboard, resize_keyboard=True))
    ...
    async def handle_remove_selected(message: Message):
        ...
        try:
            ...
            await ctx.send_message_safe(chat_id=message.chat.id, text="✅ Канал удалён", reply_markup=ctx.get_main_menu())
        except Exception as exc:
            await ctx.send_message_safe(chat_id=message.chat.id, text=f"❌ Ошибка удаления: {exc}", reply_markup=ctx.get_main_menu())
        finally:
