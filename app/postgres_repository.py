...
class PostgresRepository(RepositoryProtocol):
    ...
    def get_rule_card_snapshot(self, rule_id: int):
        """
        Единый быстрый snapshot для карточки правила.

        Что делает:
        - читает базовую строку правила + raw counters deliveries
        - одним builder-проходом считает logical_total / logical_completed / logical_pending / logical_current_position
        - возвращает ОДНУ готовую row-like dict структуру для карточки

        Это убирает двойной тяжёлый расчёт:
        - get_rule_position_info()
        - get_rule_queue_logical_items()

        И заменяет его одним snapshot-расчётом.
        """
        try:
            with self.connect() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        SELECT
                            ...
                            r.caption_delivery_mode,
                            r.video_caption_delivery_mode,

                            r.is_active,
                            CAST(r.next_run_at AS TEXT) AS next_run_at,
                            CAST(r.last_sent_at AS TEXT) AS last_sent_at,
                            r.created_date,
                    ...
                    if not base_row:
                        logger.warning(
                            "get_rule_card_snapshot: правило не найдено, rule_id=%s",
                            rule_id,
                        )
                        return None
            ...
            row = dict(base_row)
            mode = (row.get("mode") or "repost").strip().lower()
            ...
            logical_pending = sum(1 for item in logical_items if not item.get("is_done"))
            ...
            row["logical_total"] = logical_total
            row["logical_completed"] = logical_completed
            row["logical_pending"] = logical_pending
            row["logical_current_position"] = logical_current_position
            ...
            logger.exception(
                "get_rule_card_snapshot: авария snapshot карточки, rule_id=%s, error=%s",
                rule_id,
                exc,
            )
            return None
