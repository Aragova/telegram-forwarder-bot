...
def _normalize_executor_payload(job_type: str, payload: dict) -> dict:
    base_keys = {
        "rule_id",
        "delivery_id",
        "delivery_ids",
    ...
    if job_type == "repost_album":
        return {k: payload.get(k) for k in base_keys if k in payload}
    if job_type in {"repost_single", "video_delivery"}:
        return {k: payload.get(k) for k in base_keys if k in payload}
    return dict(payload)
...
async def _run_one_job(repo, sender_service, worker_id: str, queue: str) -> bool:
    ...
    payload = _normalize_executor_payload(job_type, _safe_payload(job))
