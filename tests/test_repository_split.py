...
class _FakeCursor:
    def __init__(self):
        self.updated_with: tuple[int, int] | None = None
    ...
    def execute(self, sql: str, params):
        if "UPDATE routing" in sql:
            self.updated_with = (int(params[0]), int(params[1]))

    def fetchone(self):
        return {"tenant_id": None, "created_by": 777}
...
class _FakeRoot:
    def __init__(self):
        self.conn = _FakeConn()
        self.ensured_admin: int | None = None

    def connect(self):
        return self.conn

    def _fetch_inserted_id(self, conn, sql, params):
        return None

    def _ensure_tenant_for_admin_conn(self, conn, admin_id: int):
        self.ensured_admin = int(admin_id)
        return 42


def test_usage_repository_resolves_legacy_rule_tenant_from_created_by():
    root = _FakeRoot()
    usage_repo = UsageRepository(root)

    tenant_id = usage_repo.get_rule_tenant_id(99)

    assert tenant_id == 42
    assert root.ensured_admin == 777
    assert root.conn.cursor_obj.updated_with == (42, 99)
    assert root.conn.committed is True
