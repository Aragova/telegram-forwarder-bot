...
def _get_user_state(user_id: int | None) -> dict[str, Any]:
    if user_id is None:
        return {}
    return user_states.get(int(user_id)) or {}


def _is_waiting_vip_scheduled_post_material(user_id: int | None) -> bool:
    return _get_user_state(user_id).get("state") == "waiting_vip_scheduled_post_material"
...
async def handle_user_status_callback(callback: CallbackQuery):
    ...
    if saved_post_id:
        ...
        if saved_post:
            ...
            saved_post_line = f"📝 Рекламный пост: #{saved_post_id} · {get_saved_post_short_description(content)}"
    text = (
        "📊 Живой статус\n\n"
...
async def _handle_vip_scheduled_post_material_message(message: Message) -> bool:
    user_id = message.from_user.id if message.from_user else None
    ...
    if getattr(message, "media_group_id", None):
        ...
        async def _on_album_ready(*, admin_id: int, messages: list[Message]):
            state_now = _get_user_state(admin_id)
...
@dp.message(lambda m: (
    m.chat.type == "private"
    and m.from_user
    and _is_waiting_vip_scheduled_post_material(m.from_user.id)
))
async def handle_vip_scheduled_post_material_message(message: Message):
    logger.info(
    ...
    await _handle_vip_scheduled_post_material_message(message)
...
async def edit_message_reply_markup_safe(*, message: Message, reply_markup=None) -> bool:
    ...
    if ui_policy is None:
        try:
            ...
            if "message is not modified" in str(exc).lower():
                return True
            logger.exception("edit_message_reply_markup_safe fallback failed: %s", exc)
            return False

    result = await ui_policy.edit_message_reply_markup(
        chat_id=message.chat.id,
        message_id=message.message_id,
        reply_markup=reply_markup,
    )
...
async def handle_rule_repost_campaign_schedule_input(callback: CallbackQuery):
    ...
    await edit_message_text_safe(message=callback.message, text="🧙 VIP-запуск по расписанию · Шаг 4/4\n\nВведите дату и время запуска\n\nФормат:\n09.05 18:00\n\nЧасовой пояс: UTC+3", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад к выбору времени", callback_data=f"rule_repost_campaign_schedule_step4:{rule_id}")]]))
    ...
    *,
    ...
    try:
        ...
        if force_new_message:
            ...
            logger.info("REPOST_CAMPAIGN_POST_MENU_RENDER_EDIT | rule_id=%s", rule_id)
            await edit_message_text_safe(
                message=callback.message,
                text=text,
                reply_markup=keyboard,
...
async def handle_rule_repost_campaign_post_menu(callback: CallbackQuery):
    ...
    try:
        ...
        await answer_callback_safe(callback, "Ошибка данных", show_alert=True)
        return
    force_new = _should_answer_new_message_for_callback(callback)
    if not await _render_repost_campaign_post_menu(callback, rule_id, force_new_message=force_new):
        return
    await answer_callback_safe_once(callback)
...
async def handle_rule_repost_campaign_post_unlink(callback: CallbackQuery):
    ...
    try:
        ...
        await _render_repost_campaign_post_menu(
            callback,
            rule_id,
            force_new_message=_should_answer_new_message_for_callback(callback),
        )
...
@dp.message(
    lambda m: (
        ...
        and (m.text is not None or user_states.get(m.from_user.id, {}).get("state") == "awaiting_repost_campaign_saved_post")
...
async def handle_stateful_private_inputs(message: Message):
    ...
    if state.get("state") == "awaiting_repost_campaign_saved_post":
        if _is_waiting_vip_scheduled_post_material(user_id):
            return
        rule_id = int(state.get("rule_id") or 0)
    ...
    if state.get("state") == "waiting_repost_campaign_scheduled_post_time":
        ...
        if parsed is None:
            ...
            return
        service = _build_repost_campaign_scheduled_post_service()
        await run_db(service.update_draft_scheduled_at, scheduled_post_id=scheduled_post_id, scheduled_at_utc=parsed, actor_id=message.from_user.id if message.from_user else None)
        row = await run_db(db.get_campaign_scheduled_post, scheduled_post_id)
        targets = await run_db(db.list_campaign_scheduled_post_targets, scheduled_post_id)
        readiness = await run_db(service.build_readiness, scheduled_post_id=scheduled_post_id)
...
async def _init_sender_runtime(*, create_ui_policy: bool) -> None:
    ...
    if user_handlers_ctx is not None:
        user_handlers_ctx.bot = bot
...
def _register_user_saas_handlers() -> None:
    global user_handlers_ctx
    ctx = UserHandlersContext(
        bot=bot,
        db=db,
        ...
        edit_message_reply_markup_safe=edit_message_reply_markup_safe,
        with_recovery_button=_with_recovery_button,
        recovery_has_items=_recovery_has_items,
        rules_page_size=RULES_PAGE_SIZE,
        get_user_invoices_payload=_get_user_invoices_payload,
        invoice_plan_name=_invoice_plan_name,
        public_invoice_keyboard=_public_invoice_keyboard,
        ...
        build_rule_card_payload_cached=build_rule_card_payload_cached,
        build_rule_extra_keyboard=build_rule_extra_keyboard,
        compact_rule_text=compact_rule_text,
...
async def _build_vip_scheduled_known_targets(rule_id: int, scheduled_post_id: int) -> list[dict]:
    ...
    manual_targets = await run_db(db.list_rule_repost_campaign_targets, rule_id, True) or []
    ...
    for post in posts:
        ...
        if post_id <= 0:
            continue
        scheduled_targets = await run_db(db.list_campaign_scheduled_post_targets, post_id, True) or []
...
async def handle_rule_repost_campaign_scheduled_post_new(callback: CallbackQuery):
    ...
    if not created.ok:
        await answer_callback_safe(callback, created.error_text or 'Ошибка', show_alert=True)
        return
    scheduled_post_id = int((created.extra or {}).get("scheduled_post_id") or 0)
    user_states[callback.from_user.id] = {
        "state": "waiting_vip_scheduled_post_material",
        "rule_id": rule_id,
        "scheduled_post_id": scheduled_post_id,
    }
    t = (
    ...
    k = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="⬅️ Отменить", callback_data=f"rule_repost_campaign_scheduled_post_cancel_confirm:{rule_id}:{scheduled_post_id}")
    ]])
...
async def _open_vip_scheduled_post_pick_targets(*, callback: CallbackQuery, rule_id: int, scheduled_post_id: int, page: int = 0):
    ...
    selected_targets = await run_db(db.list_campaign_scheduled_post_targets, scheduled_post_id, True) or []
...
async def handle_vip_scheduled_post_add_known_all(callback: CallbackQuery):
    ...
    await _open_vip_scheduled_post_step_targets_message(callback, rule_id=rule_id, scheduled_post_id=scheduled_post_id)
